...
public class PersonAndPetKataTest {
    ...
    public void doAnyPeopleHaveCats() {
        ...
        boolean resultLazyMR = this.people.asLazy().anySatisfyWith(Person::hasPet, PetType.CAT);
    ...
    public void doAllPeopleHaveCats() {
        ...
        boolean resultLazy = this.people.asLazy().allSatisfy(person -> person.hasPet(PetType.CAT));
        Assert.assertFalse(resultLazy);
        boolean resultLazyMR = this.people.asLazy().allSatisfyWith(Person::hasPet, PetType.CAT);
    ...
    public void doNoPeopleHaveCats() {
        ...
        Assert.assertFalse(resultLazy);
        boolean resultLazyMR = this.people.asLazy().noneSatisfyWith(Person::hasPet, PetType.CAT);
        Assert.assertFalse(resultLazyMR);
    }
    ...
    public void howManyPeopleHaveCats() {
        ...
        int countLazyMR = this.people.asLazy().countWith(Person::hasPet, PetType.CAT);
    ...
    public void getPeopleWithCats() {
        MutableList<Person> peopleWithCatsEager = this.people.select(person -> person.hasPet(PetType.CAT));
        Verify.assertSize(2, peopleWithCatsEager);
        MutableList<Person> peopleWithCatsEagerMR = this.people.selectWith(Person::hasPet, PetType.CAT);
        Verify.assertSize(2, peopleWithCatsEagerMR);
        MutableList<Person> peopleWithCatsLazy = this.people.asLazy().select(person -> person.hasPet(PetType.CAT)).toList();
        Verify.assertSize(2, peopleWithCatsLazy);
        MutableList<Person> peopleWithCatsLazyMR = this.people.asLazy().selectWith(Person::hasPet, PetType.CAT).toList();
        Verify.assertSize(2, peopleWithCatsLazyMR);
    }

    @Test
    public void getPeopleWithCatsUsingStreams() {
        List<Person> peopleWithCatsStream = this.people.stream().filter(person -> person.hasPet(PetType.CAT)).collect(Collectors.toList());
        Verify.assertSize(2, peopleWithCatsStream);
    }

    @Test
    public void getPeopleWhoDontHaveCats() {
        MutableList<Person> peopleWithNoCatsEager = this.people.reject(person -> person.hasPet(PetType.CAT));
        Verify.assertSize(5, peopleWithNoCatsEager);
        MutableList<Person> peopleWithNoCatsEagerMR = this.people.rejectWith(Person::hasPet, PetType.CAT);
        Verify.assertSize(5, peopleWithNoCatsEagerMR);
        MutableList<Person> peopleWithNoCatsLazy = this.people.asLazy().reject(person -> person.hasPet(PetType.CAT)).toList();
        Verify.assertSize(5, peopleWithNoCatsLazy);
        MutableList<Person> peopleWithNoCatsLazyMR = this.people.asLazy().rejectWith(Person::hasPet, PetType.CAT).toList();
        Verify.assertSize(5, peopleWithNoCatsLazyMR);
    }

    @Test
    public void getPeopleWhoDontHaveCatsUsingStreams() {
        List<Person> peopleWithNoCatsStream = this.people.stream().filter(person -> !person.hasPet(PetType.CAT)).collect(Collectors.toList());
    ...
    public void partitionPeopleByCatOwnersAndNonCatOwnersUsingStreams() {
        Map<Boolean, List<Person>> catsAndNoCatsStream = this.people.stream().collect(Collectors.partitioningBy(person -> person.hasPet(PetType.CAT)));
    ...
    public void findPersonNamedMarySmith() {
        ...
        Person resultLazy = this.people.asLazy().detect(person -> person.named("Mary Smith"));
        Assert.assertEquals("Mary", resultLazy.getFirstName());
        Assert.assertEquals("Smith", resultLazy.getLastName());
        Person resultLazyMR = this.people.asLazy().detectWith(Person::named, "Mary Smith");
    ...
    public void findPersonNamedMarySmithUsingStreams() {
        Person resultStream = this.people.stream().filter(person -> person.named("Mary Smith")).findFirst().orElse(null);
    ...
    public void getTheNamesOfBobSmithPetsUsingStreams() {
        Person personStream = this.people.stream().filter(each -> each.named("Bob Smith")).findFirst().orElse(null);
        List<String> names = personStream.getPets().stream().map(Pet::getName).collect(Collectors.toList());
    ...
    public void getAllPetTypes() {
        MutableSet<PetType> allPetTypesEager = this.people.flatCollect(Person::getPetTypes).toSet();
        MutableSet<PetType> expected = Sets.mutable.with(PetType.values());
        Assert.assertEquals(expected, allPetTypesEager);
        MutableSet<PetType> allPetTypesEagerTarget = this.people.flatCollect(Person::getPetTypes, Sets.mutable.empty());
        Assert.assertEquals(expected, allPetTypesEagerTarget);
        MutableSet<PetType> allPetTypesLazy = this.people.asLazy().flatCollect(Person::getPetTypes).toSet();
        Assert.assertEquals(expected, allPetTypesLazy);
        MutableSet<PetType> allPetTypesLazyTarget = this.people.asLazy().flatCollect(Person::getPetTypes, Sets.mutable.empty());
        Assert.assertEquals(expected, allPetTypesLazyTarget);
    }

    @Test
    public void getAllPetTypesUsingStreams() {
        Set<PetType> allPetTypesStream = this.people.stream().flatMap(person -> person.getPetTypes().stream()).collect(Collectors.toSet());
        Assert.assertEquals(new HashSet<>(Arrays.asList(PetType.values())), allPetTypesStream);
    }

    @Test
    public void groupPeopleByLastName() {
        MutableListMultimap<String, Person> byLastNameEager = this.people.groupBy(Person::getLastName);
        Verify.assertIterableSize(3, byLastNameEager.get("Smith"));
        MutableBagMultimap<String, Person> byLastNameEagerTargetBag = this.people.groupBy(Person::getLastName, Multimaps.mutable.bag.empty());
        Verify.assertIterableSize(3, byLastNameEagerTargetBag.get("Smith"));
        Multimap<String, Person> byLastNameLazy = this.people.asLazy().groupBy(Person::getLastName);
        Verify.assertIterableSize(3, byLastNameLazy.get("Smith"));
        MutableBagMultimap<String, Person> byLastNameLazyTargetBag = this.people.asLazy().groupBy(Person::getLastName, Multimaps.mutable.bag.empty());
        Verify.assertIterableSize(3, byLastNameLazyTargetBag.get("Smith"));
    }

    @Test
    public void groupPeopleByLastNameUsingStreams() {
        Map<String, List<Person>> byLastNameStream = this.people.stream().collect(Collectors.groupingBy(Person::getLastName));
        Verify.assertIterableSize(3, byLastNameStream.get("Smith"));
        Map<String, MutableBag<Person>> byLastNameStreamTargetBag = this.people.stream().collect(Collectors.groupingBy(Person::getLastName, Collectors.toCollection(Bags.mutable::empty)));
        Verify.assertIterableSize(3, byLastNameStreamTargetBag.get("Smith"));
    }

    @Test
    public void groupPeopleByTheirPets() {
        Multimap<PetType, Person> peopleByPetsEager = this.people.groupByEach(Person::getPetTypes);
        RichIterable<Person> catPeople = peopleByPetsEager.get(PetType.CAT);
        Assert.assertEquals("Mary, Bob", catPeople.collect(Person::getFirstName).makeString());
        RichIterable<Person> dogPeople = peopleByPetsEager.get(PetType.DOG);
        Assert.assertEquals("Bob, Ted", dogPeople.collect(Person::getFirstName).makeString());
    }
    ...
    public void testStrings() {
        Assert.assertEquals("HELLO", "h1e2l3l4o".chars().filter(Character::isLetter).map(Character::toUpperCase).collect(StringBuilder::new, StringBuilder::appendCodePoint, StringBuilder::append).toString());
        Assert.assertEquals("HELLO", "h1e2l3l4o".codePoints().filter(Character::isLetter).map(Character::toUpperCase).collect(StringBuilder::new, StringBuilder::appendCodePoint, StringBuilder::append).toString());
    ...
    public void getCountsByPetType() {
        Bag<PetType> counts = this.people.countByEach(Person::getPetTypes).toImmutable();
    ...
    public void getTop3Pets() {
        ListIterable<ObjectIntPair<PetType>> top3 = this.people.countByEach(Person::getPetTypes).topOccurrences(3);
        Verify.assertSize(3, top3);
        Assert.assertTrue(top3.containsAllArguments(PrimitiveTuples.pair(PetType.CAT, 2), PrimitiveTuples.pair(PetType.DOG, 2), PrimitiveTuples.pair(PetType.HAMSTER, 2)));
    }
    ...
    public void getBottom3Pets() {
        MutableList<ObjectIntPair<PetType>> leastFavoritesLazy = this.people.asLazy().flatCollect(Person::getPets).collect(Pet::getType).toBag().bottomOccurrences(3);
    ...
    public void getCountsByPetAge() {
        ImmutableIntBag countsLazy = this.people.flatCollectInt(Person::getPetAges, new IntHashBag()).toImmutable();
    ...
    public static class _Benchmark {
        ...
        private void runBenchmark(se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> payload) throws java.lang.Throwable {
            this.instance = new PersonAndPetKataTest();
            this.instance.setUp();
        ...
        private static class _Payloads {
            ...
            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> doAnyPeopleHaveCats;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> doAnyPeopleHaveCatsUsingStreams;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> doAllPeopleHaveCats;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> doAllPeopleHaveCatsUsingStreams;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> doNoPeopleHaveCats;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> doNoPeopleHaveCatsUsingStreams;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> howManyPeopleHaveCats;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> howManyPeopleHaveCatsUsingStreams;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> getPeopleWithCats;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> getPeopleWithCatsUsingStreams;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> getPeopleWhoDontHaveCats;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> getPeopleWhoDontHaveCatsUsingStreams;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> partitionPeopleByCatOwnersAndNonCatOwners;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> partitionPeopleByCatOwnersAndNonCatOwnersUsingStreams;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> findPersonNamedMarySmith;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> findPersonNamedMarySmithUsingStreams;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> getTheNamesOfBobSmithPets;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> getTheNamesOfBobSmithPetsUsingStreams;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> getAllPetTypes;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> getAllPetTypesUsingStreams;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> groupPeopleByLastName;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> groupPeopleByLastNameUsingStreams;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> groupPeopleByTheirPets;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> groupPeopleByTheirPetsUsingStreams;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> getTotalNumberOfPets;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> getTotalNumberOfPetsUsingStreams;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> testStrings;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> getAgeStatisticsOfPets;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> getAgeStatisticsOfPetsUsingStreams;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> getCountsByPetType;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> getCountsByPetTypeUsingStreams;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> getTop3Pets;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> getTop3PetsUsingStreams;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> getBottom3Pets;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> getCountsByPetAge;

            public se.chalmers.ju2jmh.api.ThrowingConsumer<PersonAndPetKataTest> getCountsByPetAgeUsingStreams;
        }

        @org.openjdk.jmh.annotations.Setup(org.openjdk.jmh.annotations.Level.Trial)
        public void makePayloads() {
            this.payloads = new _Payloads();
            this.payloads.doAnyPeopleHaveCats = PersonAndPetKataTest::doAnyPeopleHaveCats;
            this.payloads.doAnyPeopleHaveCatsUsingStreams = PersonAndPetKataTest::doAnyPeopleHaveCatsUsingStreams;
            this.payloads.doAllPeopleHaveCats = PersonAndPetKataTest::doAllPeopleHaveCats;
            this.payloads.doAllPeopleHaveCatsUsingStreams = PersonAndPetKataTest::doAllPeopleHaveCatsUsingStreams;
            this.payloads.doNoPeopleHaveCats = PersonAndPetKataTest::doNoPeopleHaveCats;
            this.payloads.doNoPeopleHaveCatsUsingStreams = PersonAndPetKataTest::doNoPeopleHaveCatsUsingStreams;
            this.payloads.howManyPeopleHaveCats = PersonAndPetKataTest::howManyPeopleHaveCats;
            this.payloads.howManyPeopleHaveCatsUsingStreams = PersonAndPetKataTest::howManyPeopleHaveCatsUsingStreams;
            this.payloads.getPeopleWithCats = PersonAndPetKataTest::getPeopleWithCats;
            this.payloads.getPeopleWithCatsUsingStreams = PersonAndPetKataTest::getPeopleWithCatsUsingStreams;
            this.payloads.getPeopleWhoDontHaveCats = PersonAndPetKataTest::getPeopleWhoDontHaveCats;
            this.payloads.getPeopleWhoDontHaveCatsUsingStreams = PersonAndPetKataTest::getPeopleWhoDontHaveCatsUsingStreams;
            this.payloads.partitionPeopleByCatOwnersAndNonCatOwners = PersonAndPetKataTest::partitionPeopleByCatOwnersAndNonCatOwners;
            this.payloads.partitionPeopleByCatOwnersAndNonCatOwnersUsingStreams = PersonAndPetKataTest::partitionPeopleByCatOwnersAndNonCatOwnersUsingStreams;
            this.payloads.findPersonNamedMarySmith = PersonAndPetKataTest::findPersonNamedMarySmith;
            this.payloads.findPersonNamedMarySmithUsingStreams = PersonAndPetKataTest::findPersonNamedMarySmithUsingStreams;
            this.payloads.getTheNamesOfBobSmithPets = PersonAndPetKataTest::getTheNamesOfBobSmithPets;
            this.payloads.getTheNamesOfBobSmithPetsUsingStreams = PersonAndPetKataTest::getTheNamesOfBobSmithPetsUsingStreams;
            this.payloads.getAllPetTypes = PersonAndPetKataTest::getAllPetTypes;
            this.payloads.getAllPetTypesUsingStreams = PersonAndPetKataTest::getAllPetTypesUsingStreams;
            this.payloads.groupPeopleByLastName = PersonAndPetKataTest::groupPeopleByLastName;
            this.payloads.groupPeopleByLastNameUsingStreams = PersonAndPetKataTest::groupPeopleByLastNameUsingStreams;
            this.payloads.groupPeopleByTheirPets = PersonAndPetKataTest::groupPeopleByTheirPets;
            this.payloads.groupPeopleByTheirPetsUsingStreams = PersonAndPetKataTest::groupPeopleByTheirPetsUsingStreams;
            this.payloads.getTotalNumberOfPets = PersonAndPetKataTest::getTotalNumberOfPets;
            this.payloads.getTotalNumberOfPetsUsingStreams = PersonAndPetKataTest::getTotalNumberOfPetsUsingStreams;
            this.payloads.testStrings = PersonAndPetKataTest::testStrings;
            this.payloads.getAgeStatisticsOfPets = PersonAndPetKataTest::getAgeStatisticsOfPets;
            this.payloads.getAgeStatisticsOfPetsUsingStreams = PersonAndPetKataTest::getAgeStatisticsOfPetsUsingStreams;
            this.payloads.getCountsByPetType = PersonAndPetKataTest::getCountsByPetType;
            this.payloads.getCountsByPetTypeUsingStreams = PersonAndPetKataTest::getCountsByPetTypeUsingStreams;
            this.payloads.getTop3Pets = PersonAndPetKataTest::getTop3Pets;
            this.payloads.getTop3PetsUsingStreams = PersonAndPetKataTest::getTop3PetsUsingStreams;
            this.payloads.getBottom3Pets = PersonAndPetKataTest::getBottom3Pets;
            this.payloads.getCountsByPetAge = PersonAndPetKataTest::getCountsByPetAge;
